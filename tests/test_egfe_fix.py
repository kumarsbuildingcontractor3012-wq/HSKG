...
This test was moved from the repository root into `tests/` for proper structure.
"""
...
egfe_dir = Path(__file__).resolve().parents[1] / "tests" / "fixtures" / "EGFE-dataset"
if egfe_dir.exists():
...
csv_path = Path(__file__).resolve().parents[1] / "tests" / "fixtures" / "Ux_data.csv"
pdf_path = Path(__file__).resolve().parents[1] / "tests" / "fixtures" / "100_websites_compressed_11zon.pdf"
...
print("=" * 70)
all_design_concepts = (design_concepts if 'design_concepts' in locals() else []) + egfe_concepts
all_concepts = (ux_concepts if 'ux_concepts' in locals() else []) + all_design_concepts

print(f"UX Concepts:                 {len(ux_concepts) if 'ux_concepts' in locals() else 0}")
print(f"Design Concepts (PDF):       {len(design_concepts) if 'design_concepts' in locals() else 0}")
...
print(f"  UX concepts (expected ~5000):         {len(ux_concepts) if 'ux_concepts' in locals() else 0}")
print(f"  Design concepts (expected >4000):     {len(all_design_concepts)}")
print(f"  Total concepts (expected >9000):      {len(all_concepts)}")

print("EGFE concept extraction quick test complete.")
